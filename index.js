  ...
  try {
    if (improvement && relevant.length) {
      ...
      if (r && r.opened && r.opened.length)
        gradDirective = ` When the task ends, call mikey_graduation_track for each of these you actually followed (${r.opened.join(', ')}) with success true/false, or applicable:false if it did not apply to this turn — the row is already open and waiting for its outcome.`;
    }
